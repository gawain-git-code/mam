...
int DATA_PORT=SYS_PORT + 1; //15556;
int LIVE_PORT=SYS_PORT + 2; //15557;
int STR_PORT=SYS_PORT + 3; //15558;
int INDICATOR_DATA_PORT=SYS_PORT + 4; //15559;
int CHART_DATA_PORT=SYS_PORT + 5; //15560;
int CHART_INDICATOR_DATA_PORT=SYS_PORT + 6; //15562;
               ...
               if(action=="TRADE")
                  ...
                  if(action=="POSITIONS")
                     ...
                     if(action=="ORDERS")
                        ...
                        if(action=="NOTIFY")
                           PushNotification(incomingMessage);
                        else
                           if(action=="RESET")
                              ResetSubscriptionsAndIndicators();
                           else
...
//| Push notification                                                |
//+------------------------------------------------------------------+
void PushNotification(CJAVal &dataObject)
  {

   string actionType = dataObject["actionType"].ToStr();
   string msg = dataObject["comment"].ToStr();
   if(debug)
      Print("Receive notification: ", msg);
   bool ret = SendNotification(msg);
   //CheckError(__FUNCTION__);
   if (debug && ret == false)
      Print("PushNotification Error: ", GetLastError());
   CJAVal info;
   info["error"] = !ret;
   string t=info.Serialize();
   if(debug)
